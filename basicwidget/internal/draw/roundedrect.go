...
func DrawRoundedRectWithSharpCorners(context *guigui.Context, dst *ebiten.Image, bounds image.Rectangle, clr color.Color, radius int, sharpCorners Corners) {
	...
	DrawNinePatchParts(dst, bounds, ensureWhiteRoundedRect(radius), clr, clr, sharpCorners.bools())
	if !dst.Bounds().Intersect(bounds).Empty() {
		theNinePatchVertices, theNinePatchIndices = AppendNinePatchVertices(theNinePatchVertices[:0], theNinePatchIndices[:0], bounds, whiteImage.Bounds(), radius, radius, clr, clr, sharpCorners.invertedBools())
...
func DrawRoundedRectBorderWithSharpCorners(context *guigui.Context, dst *ebiten.Image, bounds image.Rectangle, clr1, clr2 color.Color, radius int, borderWidth float32, borderType RoundedRectBorderType, sharpCorners Corners) {
	...
	DrawNinePatchParts(dst, bounds, ensureWhiteRoundedRectBorder(radius, borderWidth, borderType, context.ColorMode()), clr1, clr2, sharpCorners.bools())
	DrawNinePatchParts(dst, bounds, ensureWhiteRectBorder(radius, borderWidth, borderType, context.ColorMode()), clr1, clr2, sharpCorners.invertedBools())
}
