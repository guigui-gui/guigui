...
func TestPieceTableWriteRangeToWithInsertionFull(t *testing.T) {
	tests := []struct {
		name  string
		init  string
		text  string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			p.Replace(tc.init, 0, 0)
			var b strings.Builder
			if _, err := p.WriteRangeToWithInsertion(&b, tc.text, tc.start, tc.end, 0, math.MaxInt); err != nil {
			...
			if _, err := p.WriteRangeTo(&b2, 0, math.MaxInt); err != nil {
				t.Fatalf("WriteRangeTo failed: %v", err)
			}
...
func TestPieceTableWriteRangeTo(t *testing.T) {
	...
	p.UpdateByIME("!", 12, 12) // "Hello, there!"
...
func TestPieceTableWriteRangeRoundTrip(t *testing.T) {
	...
	if _, err := p.WriteRangeTo(&ref, 0, math.MaxInt); err != nil {
		t.Fatalf("WriteRangeTo failed: %v", err)
	}
	full := ref.String()
...
func TestPieceTableWriteRangeToWithInsertion(t *testing.T) {
	...
	newPT := func() *piecetable.PieceTable {
		var p piecetable.PieceTable
		p.Replace("Hello World", 0, 0)
		p.Replace(", ", 5, 6)
		p.Replace("there", 7, 12)
...
func TestPieceTableWriteRangeToWithInsertionRoundTrip(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			// Build the full reference output via the unbounded range.
			var ref strings.Builder
			if _, err := p.WriteRangeToWithInsertion(&ref, c.text, c.insertStart, c.insertEnd, 0, math.MaxInt); err != nil {
				t.Fatalf("WriteRangeToWithInsertion failed: %v", err)
			}
			full := ref.String()
			for start := 0; start <= len(full); start++ {
...
func TestPieceTableReadFrom(t *testing.T) {
	check := func(t *testing.T, p *piecetable.PieceTable, want string) {
		t.Helper()
		var b strings.Builder
		if _, err := p.WriteRangeTo(&b, 0, math.MaxInt); err != nil {
			t.Fatalf("WriteRangeTo failed: %v", err)
