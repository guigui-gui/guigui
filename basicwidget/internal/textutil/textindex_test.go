...
func TestTextIndexFromPositionLineOffsetsWordWrap(t *testing.T) {
	...
	totalVL := textutil.MeasureHeight(narrowWidth, str, textutil.WrapModeNormal, face, nil, textutil.Insertion{}, lineHeight, textutil.LineHeightModeFixed, 0, false) / lineHeight
...
func TestTextIndexFromPositionHintParity(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name+wrapModeSuffix(tc.wrapMode), func(t *testing.T) {
			...
			totalVL := int(textutil.MeasureHeight(tc.width, tc.str, tc.wrapMode, face, nil, textutil.Insertion{}, lineHeight, textutil.LineHeightModeFixed, 0, false) / lineHeight)
